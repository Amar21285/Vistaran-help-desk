...
try {
    ...
    // Safety check for common trailing characters in some environments
    ...
    for (const [key, filename] of Object.entries(keyToFileMap)) {
        if (backupData[key]) {
            ...
            console.log(`✅ Saved ${filename} (${recordCount} records/data)`);
            updatedCount++;
        } else {
    ...
    console.log(`\n✨ Data import complete! ${updatedCount} files updated.`);
} catch (error) {
