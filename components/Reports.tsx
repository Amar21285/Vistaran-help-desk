...
const Reports: React.FC<ReportsProps> = ({ 
    ...
    const currentMetrics = useMemo(() => {
        switch(activeTab) {
            ...
            case 'petty-cash': return { total: filteredPettyCash.length, label: 'Claims', secondary: `₹${filteredPettyCash.reduce((acc, r) => acc + (parseFloat(String(r.amount).replace(/,/g, '')) || 0), 0).toLocaleString()}`, sLabel: 'Expenditure' };
            case 'internet': return { total: filteredInternet.length, label: 'Links', secondary: `₹${filteredInternet.reduce((acc, v) => acc + (parseFloat(String(v.amount).replace(/,/g, '')) || 0), 0).toLocaleString()}`, sLabel: 'Sub Cost' };
